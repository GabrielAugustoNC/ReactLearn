...
import { ArgumentScale, Stack, Animation, EventTracker, HoverState, SelectionState } from '@devexpress/dx-react-chart';
import Button from '@material-ui/core/Button';
import { withStyles } from '@material-ui/core/styles';
import { NavigateBefore, NavigateNext } from '@material-ui/icons/';
import Typography from '@material-ui/core/Typography';
import { annualVehiclesSales as data } from './Data/VisualizationProductChart';
import {
  Chart,
  BarSeries,
  ArgumentAxis,
...
const formatInfo = (target) => {
  ...
  return `${series} vendeu R$ ${value} em ${argument}`;
};
  ...
  classes, target, turnNext, turnPrev, clear,
    ...
    <div>
      ...
      <div className={classes.text}>
        <Typography color="textSecondary" variant="body2" className={classes.name}>Selecionado:</Typography>
  ...
  classes, target, enabled, toggle,
    ...
    <div className={classes.hoverGroup}>
      ...
      <div className={classes.text}>
        ...
        <Typography>{formatInfo(target) === 'None' || !enabled ? '' : formatInfo(target)}</Typography>
      </div>
    </div>
  );
...
class Demo extends React.PureComponent {
  constructor(props) {
    ...
    this.state = {
      hover: null,
      selection: [{ series: 'FestvalCWB', point: 3 }],
      tooltipTarget: null,
      tooltipEnabled: true,
    };
    ...
    return (
      <Paper>
        ...
        >
          ...
          <BarSeries
            name="FestvalCWB"
            valueField="FestvalCWB"
            argumentField="year"
          />
          <BarSeries
            name="FestvalPR"
            valueField="FestvalPR"
            argumentField="year"
          />
          <Stack />
